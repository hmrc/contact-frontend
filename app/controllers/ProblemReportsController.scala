...
object ProblemReportForm {
  ...
  def form(implicit request: Request[_], appConfig: AppConfig) =
    Form[ProblemReport](
      mapping(
        "report-name" -> text
          .verifying(
...
class ProblemReportsController @Inject()(
  ...
  ticketCreatedBody_B: ticket_created_body_b)(implicit appConfig: AppConfig, val executionContext: ExecutionContext)
  ...
  def reportForm(secure: Option[Boolean], preferredCsrfToken: Option[String], service: Option[String]) = Action {
    implicit request =>
      ...
      val postEndpoint =
        if (isSecure) appConfig.externalReportProblemSecureUrl
        else appConfig.externalReportProblemUrl
      ...
      (error: Form[ProblemReport]) => {
        ...
        if (isAjax) {
          if (appConfig.hasFeature(GetHelpWithThisPageOnlyServerSideValidation, error.data.get("service"))) {
            val csrfToken =
              play.filters.csrf.CSRF.getToken(request).map(_.value)
            Future.successful(
              Ok(
                Json.toJson(Map(
                  "status"  -> "OK",
          ...
          Future.successful(
            Ok(
              problemReportsPage(error, appConfig.externalReportProblemSecureUrl, error.data.get("service"), referrer)))
        }
      },
      problemReport => {

        val referrer = problemReport.referrer
          .filter(_.trim.nonEmpty)
          .orElse(request.headers.get(REFERER))
        val futureEnrolments = maybeAuthenticatedUserEnrolments()
        val futureLoggedIn   = isAuthorised()
        (for {
          maybeUserEnrolments <- futureEnrolments
          loggedIn            <- futureLoggedIn
          ticketId            <- createProblemReportsTicket(problemReport, request, maybeUserEnrolments, referrer, loggedIn)
        } yield {
          if (isAjax) {
