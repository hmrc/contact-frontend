...
class CaptchaServiceV3 @Inject()(captchaConnector: CaptchaConnectorV3, appConfig: AppConfig, metrics: Metrics)(
  implicit ec: ExecutionContext) extends CaptchaService {
  ...
  private [services] def checkIfBot(apiResponse: CaptchaApiResponseV3): Boolean =
    apiResponse match {
      ...
      case UnsuccessfulCaptchaApiResponse(errors) =>
        logger.warn(s"reCAPTCHA v3 verification failed, " +
