...
class FieldTransformerSpec
  ...
  "Field Transformer" should {
    ...
    "transform loggedIn as false to userId n/a" in new FieldTransformerScope {
      transformer.userIdFrom(false) shouldBe "n/a"
    }

    "transform loggedIn as true to user id LoggedInUser" in new FieldTransformerScope {
      transformer.userIdFrom(true) shouldBe "LoggedInUser"
    }
...
class FieldTransformerScope {
  ...
  val hc =
    HeaderCarrier(userId = Some(userId), sessionId = Some(SessionId(sessionId)))
  ...
  lazy val requestAuthenticatedByIda = FakeRequest()
    .withHeaders(("User-Agent", userAgent))
    .withSession(("ap", "IDA"), (SessionKeys.authToken, "12345"))
