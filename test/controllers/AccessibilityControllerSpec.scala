...
class AccessibilityControllerSpec
    extends AnyWordSpec
    with Matchers
  ...
  "Accessibility form endpoint" should {
    ...
    "redirect to unauthenticated page when user isnt logged in" in new AccessibilityControllerApplication(
      ...
      val result =
        controller.accessibilityForm(service = None,
                                     userAction = Some(userAction))(request)

      status(result) should be(303)
      header(LOCATION, result) should be(Some(
        s"/contact/accessibility-unauthenticated?userAction=%2Fpage%2Ftest%3F1234%3Dxyz"))
    }

    "show the authenticated form page if logged in" in new AccessibilityControllerApplication(
      fakeApplication) {
      val request = FakeRequest()
      val result = controller.unauthenticatedAccessibilityForm(
        service = None,
        userAction = Some("test?1234=xyz"),
        referrerUrl = Some("some.referrer.url"))(request)
  ...
  "Reporting an accessibility problem without logging in" should {
    ...
    "return 200 and a valid html page for a request with optional referrerUrl" in new AccessibilityControllerApplication(
      ...
      document
        .getElementsByAttributeValue("name", "userAction")
        .first()
        .`val`() shouldBe "test?1234=xyz"
      document
        .getElementsByAttributeValue("name", "referrer")
        .first()
        .`val`() shouldBe "some.referrer.url"
    }
    ...
    "return 200 and a valid html page for a request when no referrerUrl and referer in header" in new AccessibilityControllerApplication(
      ...
      val request = FakeRequest().withHeaders((REFERER, "referrer.from.header"))
      val result =
        controller.unauthenticatedAccessibilityForm(service = None,
                                                    userAction =
                                                      Some("test?1234=xyz"),
                                                    referrerUrl = None)(request)
      ...
      document
        .getElementsByAttributeValue("name", "referrer")
        .first()
        .`val`() shouldBe "referrer.from.header"
    }
    ...
    "display errors when form isn't filled out at all" in new AccessibilityControllerApplication(
      fakeApplication) {

      val request = generateRequest(desc = "",
                                    formName = "",
                                    email = "",
                                    isJavascript = false,
                                    referrer = "/somepage")
      ...
      errors.exists(_.text().equals(
        Messages("error.common.accessibility.problem.required"))) shouldBe true
      errors.exists(_.text()
        .equals(Messages("error.common.feedback.name_mandatory"))) shouldBe true
      errors.exists(
        _.text().equals(Messages(
          "error.common.accessibility.problem.email_mandatory"))) shouldBe true
    }
    ...
    "display error messages when message size exceeds limit" in new AccessibilityControllerApplication(
      ...
      val request = generateRequest(desc = msg2500,
                                    formName = "firstname",
      ...
      errors.exists(_.text().equals(
        Messages("error.common.accessibility.problem.toolong"))) shouldBe true
    }

    "display error messages when email is invalid" in new AccessibilityControllerApplication(
      fakeApplication) {
      val badEmail = "firstname'email.gov."

      val request = generateRequest(desc = "valid form message",
                                    formName = "firstname",
                                    email = badEmail,
                                    isJavascript = false,
                                    referrer = "/somepage")
    ...
    "display error messages when email is too long" in new AccessibilityControllerApplication(
      fakeApplication) {
    ...
    "display error messages when name is too long" in new AccessibilityControllerApplication(
      fakeApplication) {
      val longName = "x" * 256
      ...
      val errors = document.getElementsByClass("error-message").asScala
      errors.exists(_.text()
        .equals(Messages("error.common.feedback.name_too_long"))) shouldBe true
    }

    "redirect to thankyou page when completed" in new AccessibilityControllerApplication(
      fakeApplication) {
      when(
        hmrcDeskproConnector.createDeskProTicket(
          ...
          any[Option[String]],
          any[Option[String]]
        )(any[HeaderCarrier])).thenReturn(Future.successful(TicketId(1234)))
      ...
      header(LOCATION, result) should be(
        Some("/contact/accessibility-unauthenticated/thanks"))
    }
  }

  "Reporting an accessibility problem when logging in" should {

    "return 200 and a valid html page for a request" in new AccessibilityControllerApplication(
      fakeApplication) {

      val request =
        FakeRequest().withSession(SessionKeys.authToken -> "authToken")
      val result = controller.accessibilityForm(
        service = None,
        userAction = Some("test?1234=xyz"))(request)
    ...
    "display errors when form isn't filled out at all" in new AccessibilityControllerApplication(
      ...
      val request = generateRequest(desc = "",
                                    ...
                                    isJavascript = false,
                                    referrer = "/somepage")
      val result = controller.submitAccessibilityForm()(
  ...
  class AccessibilityControllerApplication(app: Application)
    ...
    val accessibiltyConfirmationPage =
      app.injector.instanceOf[views.html.accessibility_confirmation]
    ...
    val controller = new AccessibilityController(
      ...
      accessibiltyPage,
      accessibiltyConfirmationPage
    )(cconfig, ExecutionContext.Implicits.global)

    def generateRequest(desc: String,
                        formName: String,
                        email: String,
