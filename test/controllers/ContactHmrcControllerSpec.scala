...
class ContactHmrcControllerSpec
  ...
  val hmrcDeskproConnectorTimout: VerificationWithTimeout =
  ...
  "ContactHmrcController" should {
    ...
    "return expected OK for non-authenticated index page" in new ContactHmrcControllerApplication {
      ...
      val contactRequest =
        FakeRequest().withHeaders((REFERER, "/some-service-page"))
      val serviceName = "my-fake-service"

      When(
        "the unauthenticated Contact HMRC page is requested with a service name")
      val contactResult =
        controller.indexUnauthenticated(serviceName, None, None)(contactRequest)
      ...
      page
        ...
        .attr("value") shouldBe "/some-service-page"
      page
        .body()
        .getElementById("service")
        .attr("value") shouldBe "my-fake-service"
    }
    ...
    "use the referrerUrl parameter if supplied" in new ContactHmrcControllerApplication {
      ...
      When(
        "the unauthenticated Contact HMRC page is requested with a service name and a referrer url")
      val contactResult =
        controller.indexUnauthenticated(serviceName, None, referrerUrl)(
          contactRequest)
    ...
    "return expected OK for non-authenticated submit page" in new ContactHmrcControllerApplication {
      ...
      val contactRequest =
        FakeRequest().withFormUrlEncodedBody(fields.toSeq: _*)
      ...
      Mockito
        .verify(hmrcDeskproConnector, hmrcDeskproConnectorTimout)
        .createDeskProTicket(
          any[String],
          any[String],
          any[String],
    ...
    "send the referrer URL to DeskPro" in new ContactHmrcControllerApplication {
      ...
      Mockito
        ...
        .createDeskProTicket(
          ...
          any[String],
          mockitoEq[String](
            "https://www.other-gov-domain.gov.uk/path/to/service/page"),
          any[Boolean],
          any[Request[AnyRef]](),
          any[Option[Enrolments]],
          any[Option[String]],
    ...
    "send the referrer information to DeskPro with userAction replacing the path if non-empty" in new ContactHmrcControllerApplication {
      ...
      Mockito
        ...
        .createDeskProTicket(
          ...
          any[String],
          mockitoEq[String](
            "https://www.other-gov-domain.gov.uk/overridden/path"),
          any[Boolean],
          any[Request[AnyRef]](),
          any[Option[Enrolments]],
          any[Option[String]],
  ...
  "Submitting contact hrmc form" should {
    "return JSON with ticket id for sucessful form submission" in new ContactHmrcControllerApplication {
      ...
      val result =
        controller.submitContactHmrcPartialForm(
          resubmitUrl = resubmitUrl,
          renderFormOnly = false)(contactRequest)
    ...
    "redisplay form in case of validation errors - rendering only form without header" in new ContactHmrcControllerApplication {
      ...
      val result =
        controller.submitContactHmrcPartialForm(
          resubmitUrl = resubmitUrl,
          renderFormOnly = true)(contactRequest)
    ...
    "show an error page with bad request HTTP code if sending the data to deskpro failed" in new ContactHmrcControllerApplication {
      ...
      page
        ...
        .text() shouldBe "Sorry, we’re experiencing technical difficulties"
    }
  }
  ...
  class ContactHmrcControllerApplication extends MockitoSugar {
    ...
    val captchaService = new CaptchaService {
      override def validateCaptcha(response: String)(
          implicit headerCarrier: HeaderCarrier): Future[Boolean] =
        Future.successful(true)
    }

    val contactPage =
      fakeApplication.injector.instanceOf[views.html.contact_hmrc]
    val contactConfirmationPage =
      fakeApplication.injector.instanceOf[views.html.contact_hmrc_confirmation]
    val contactForm =
      fakeApplication.injector.instanceOf[views.html.partials.contact_hmrc_form]
    val contactFormConfirmation =
      fakeApplication.injector
        .instanceOf[views.html.partials.contact_hmrc_form_confirmation]
    val deskproErrorPage =
      fakeApplication.injector.instanceOf[views.html.deskpro_error]
    val recaptcha =
      fakeApplication.injector.instanceOf[views.html.helpers.recaptcha]
