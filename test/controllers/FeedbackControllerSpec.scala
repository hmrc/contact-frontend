...
class FeedbackControllerSpec
  ...
  "feedbackPartialForm" should {
    ...
    "use the (deprecated) referer parameter if supplied" in new FeedbackControllerApplication(
      fakeApplication) {
      val result = controller.feedbackPartialForm(submitUrl,
                                                  csrfToken,
                                                  service,
                                                  referer,
                                                  canOmitComments,
      ...
      page
        ...
        .attr("value") shouldBe "https://www.example.com/some-service"
    }

    "use the referrerUrl parameter if supplied" in new FeedbackControllerApplication(
      fakeApplication) {
      ...
      page
        .body()
        .getElementById("referrer")
        .attr("value") shouldBe "https://www.other-example.com/some-service"
    }
  }
  ...
  "Submitting the feedback for unauthenticated user" should {
    ...
    "redirect to confirmation page without 'back' button if 'back' link not provided" in new FeedbackControllerApplication(
      ...
      redirectLocation(result) shouldBe Some(
        "/contact/beta-feedback/thanks-unauthenticated")

      verifyRequestMade()
    }

    "show errors if some form not filled in correctly" in new FeedbackControllerApplication(
      fakeApplication) {
    ...
    "succed with comment if 'canOmitComments' flag is true" in new FeedbackControllerApplication(
      fakeApplication) {
      hmrcConnectorWillReturnTheTicketId()

      val result = controller.submitUnauthenticated()(
        generateRequest(comments = "Some comment", canOmitComments = true))
    ...
    "succed without comment if 'canOmitComments' flag is true" in new FeedbackControllerApplication(
      fakeApplication) {
      hmrcConnectorWillReturnTheTicketId()

      val result = controller.submitUnauthenticated()(
        generateRequest(comments = "", canOmitComments = true))
    ...
    "fail without comment if 'canOmitComments' flag is false" in new FeedbackControllerApplication(
      fakeApplication) {
      hmrcConnectorWillReturnTheTicketId()

      val result = controller.submitUnauthenticated()(
        generateRequest(comments = "", canOmitComments = false))
    ...
    "include 'server', 'backUrl' and 'canOmitComments' fields in the returned page if form not filled in correctly" in new FeedbackControllerApplication(
      ...
      val result = controller.submitUnauthenticated()(
        generateInvalidRequestWithBackUrlAndService())
      ...
      page
        .body()
        .getElementById("feedbackService")
        .attr("value") shouldBe "someService"
      page
        .body()
        .getElementById("feedbackBackUrl")
        .attr("value") shouldBe "http://www.back.url"
      page
        .body()
        .getElementById("canOmitComments")
        .attr("value") shouldBe "true"

    }

    "show errors if call to hmrc-deskpro failed" in new FeedbackControllerApplication(
      fakeApplication) {

      hmrcConnectorWillFail()

      an[Exception] shouldBe thrownBy(
        await(controller.submitUnauthenticated()(request)))
    ...
    "redirect to confirmation page with 'back' button if 'back' link provided" in new FeedbackControllerApplication(
      ...
      status(result) should be(303)
      redirectLocation(result) shouldBe Some(
        s"/contact/beta-feedback/thanks-unauthenticated?backUrl=${URLEncoder
          .encode("http://www.back.url", "UTF-8")}")
  ...
  "Submitting feedback for authenticated user" should {
    ...
    "redirect to confirmation page with 'back' button if 'back' link provided" in new FeedbackControllerApplication(
      ...
      val result = controller.submit()(
        requestWithBackLink.withSession(SessionKeys.authToken -> "authToken"))
  ...
  "Feedback confirmation page for authenticated user " should {
    "not contain back button if not requested" in new FeedbackControllerApplication(
      fakeApplication) {

      val submit = controller.thanks()(
        request.withSession(SessionKeys.authToken -> "authToken",
                            "ticketId" -> "TID"))
    ...
    "contain back button if requested and the back url is valid" in new FeedbackControllerApplication(
      ...
      page
        .body()
        .getElementById("feedback-back")
        .attr("href") shouldBe "http://www.valid.url"
    ...
    "Feedback confirmation page for anonymous user " should {
      ...
      "contain back button if requested and the back url is valid" in new FeedbackControllerApplication(
        ...
        val submit = controller.unauthenticatedThanks(
          backUrl = Some("http://www.valid.url"))(
          request.withSession(SessionKeys.authToken -> "authToken",
                              "ticketId" -> "TID"))
        val page = Jsoup.parse(contentAsString(submit))
      ...
      "not contain back button if requested and the back url is invalid" in new FeedbackControllerApplication(
        ...
        val submit = controller.unauthenticatedThanks(
          backUrl = Some("http://www.invalid.url"))(
          request.withSession(SessionKeys.authToken -> "authToken",
                              "ticketId" -> "TID"))
        val page = Jsoup.parse(contentAsString(submit))
  ...
  "Submitting the partial feedback form" should {
    ...
    "show errors if the form is not filled in correctly" in new FeedbackControllerApplication(
      fakeApplication) {

      hmrcConnectorWillReturnTheTicketId()

      val result =
        controller.submitFeedbackPartialForm("tstUrl")(generateInvalidRequest())
    ...
    "allow comments to be empty if the canOmitComments flag is set to true" in new FeedbackControllerApplication(
      ...
      val result = controller.submitFeedbackPartialForm("tstUrl")(
        generateRequest(comments = "", canOmitComments = true))
  ...
  class FeedbackControllerApplication(app: Application) extends MockitoSugar {
    ...
    def hmrcConnectorWillReturnTheTicketId() =
    ...
    val feedbackPage =
      fakeApplication().injector.instanceOf[views.html.feedback]
    val feedbackConfirmationPage =
      fakeApplication().injector.instanceOf[views.html.feedback_confirmation]
    val feedbackPartialForm =
      fakeApplication().injector.instanceOf[views.html.partials.feedback_form]
    val feedbackFormConfirmation = fakeApplication().injector
      .instanceOf[views.html.partials.feedback_form_confirmation]
    ...
    def generateRequest(javascriptEnabled: Boolean = true,
      ...
      FakeRequest()
        .withHeaders((REFERER, feedbackReferrer),
    ...
    def generateInvalidRequestWithBackUrlAndService() =
      FakeRequest()
        .withHeaders((REFERER, feedbackReferrer),
                     ("User-Agent", "iAmAUserAgent"))
        .withFormUrlEncodedBody("isJavascript" -> "true",
    ...
    val requestWithBackLink = generateRequest(
      backUrl = Some("http://www.back.url"))
  }
