...
class AssetsFrontendProblemReportsControllerSpec extends AnyWordSpec with GuiceOneAppPerSuite with Matchers {
  ...
  "Reporting a problem" should {
    ...
    "return 200 and a valid html page for a valid request and js is not enabled for an unauthenticated user" in new TestScope {
    ...
    "return 200 and a valid html page for a valid request and js is not enabled for an authenticated user" in new TestScope {
      when(
    ...
    "return 200 and a valid json for a valid request and js is enabled" in new TestScope {
      when(
        hmrcDeskproConnector.createDeskProTicket(
          meq("John Densmore"),
          meq("name@mail.com"),
    ...
    "return 200 and a valid html page with validation error for invalid input and js is not enabled" in new TestScope {
    ...
    "return 400 and a valid json containing validation errors for invalid input and js is enabled" in new TestScope {
    ...
    "fail if the name has invalid characters - Javascript disabled" in new TestScope {
    ...
    "return error page if the Deskpro ticket creation fails - Javascript disabled" in new TestScope {
      when(
        hmrcDeskproConnector.createDeskProTicket(
          meq("John Densmore"),
          meq("name@mail.com"),
