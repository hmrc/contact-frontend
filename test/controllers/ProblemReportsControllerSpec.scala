...
class ProblemReportsControllerSpec extends AnyWordSpec with GuiceOneAppPerTest with Matchers {
  ...
  "Reporting a problem" should {
    ...
    "return 200 and a valid html page for a valid request and js is not enabled for an authenticated user" in new ProblemReportsControllerApplication(fakeApplication) {
        when(hmrcDeskproConnector.createDeskProTicket(meq("John Densmore"), meq("name@mail.com"), meq("Support Request"), meq(controller.problemMessage("Some Action", "Some Error")), meq("/contact/problem_reports"), meq(false), any[Request[AnyRef]](), meq(enrolments), meq(None), meq(None), meq(None))(any(classOf[HeaderCarrier]))).thenReturn(Future.successful(TicketId(123)))
    ...
    "return 200 and a valid json for a valid request and js is enabled" in new ProblemReportsControllerApplication(fakeApplication) {
        when(hmrcDeskproConnector.createDeskProTicket(meq("John Densmore"), meq("name@mail.com"), meq("Support Request"), meq(controller.problemMessage("Some Action", "Some Error")), meq("/contact/problem_reports"), meq(true), any[Request[AnyRef]](), meq(None), meq(None), meq(None), meq(None))(any(classOf[HeaderCarrier]))).thenReturn(Future.successful(TicketId(123)))
    ...
    "return error page if the Deskpro ticket creation fails - Javascript disabled" in new ProblemReportsControllerApplication(fakeApplication) {
        when(hmrcDeskproConnector.createDeskProTicket(meq("John Densmore"), meq("name@mail.com"), meq("Support Request"), meq(controller.problemMessage("Some Action", "Some Error")), meq("/contact/problem_reports"), meq(false), any[Request[AnyRef]](), meq(None), meq(None), meq(None), meq(None))(any(classOf[HeaderCarrier]))).thenReturn(Future.failed(new Exception("failed")))
...
class ProblemReportsControllerApplication(app: Application) extends MockitoSugar {
  ...
  private def mockHmrcConnector(result: Future[TicketId]) = {
    when(hmrcDeskproConnector.createDeskProTicket(
      ...
      meq(None),
      meq(None),
      meq(None))(any(classOf[HeaderCarrier]))).thenReturn(result)
  }
}
