...
class ProblemReportsControllerSpec
    extends AnyWordSpec
  ...
  "Reporting a problem" should {
    ...
    "fail if the email has invalid syntax (for DeskPRO)" in new ProblemReportsControllerApplication(
      ...
      val submit = controller.submit()(
        generateRequest(javascriptEnabled = false, email = "a@a"))
    ...
    "fail if the name has invalid characters - Javascript disabled" in new ProblemReportsControllerApplication(
      fakeApplication) {
      val submit = controller.submit()(
        generateRequest(javascriptEnabled = false,
...
class ProblemReportsControllerApplication(app: Application)
  ...
  val problemReportConfirmationPage_B = app.injector
    .instanceOf[views.html.problem_reports_confirmation_nonjavascript_b]
  ...
  val ticketCreatedBody_B =
    app.injector.instanceOf[views.html.ticket_created_body_b]
  ...
  def generateRequest(javascriptEnabled: Boolean = true,
    ...
    val headers = Seq((REFERER, deskproReferrer),
                      ("User-Agent", "iAmAUserAgent")) ++ Seq(
      ("X-Requested-With", "XMLHttpRequest")).filter(_ => javascriptEnabled)

    FakeRequest()
      .withHeaders(headers: _*)
      .withFormUrlEncodedBody("report-name" -> name,
                              "report-email" -> email,
  ...
  private def mockHmrcConnector(result: Future[TicketId]) = {
    when(
      hmrcDeskproConnector.createDeskProTicket(
        ...
        meq(None),
        meq(None)
      )(any(classOf[HeaderCarrier]))).thenReturn(result)
  }
}
