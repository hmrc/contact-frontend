...
class DeviceIdBucketCalculatorSpec extends AnyWordSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  implicit override val generatorDrivenConfig: PropertyCheckConfiguration =
    PropertyCheckConfiguration(minSize = 100000, sizeRange = 200000)
  ...
  "Device ID should be assigned from 1 to 99 with roughly equal probability distribution (chi-squared 99.5% certainty for 1000 degrees of freedom)" in {
    ...
    def generateTestDeviceId: FakeRequest[AnyContentAsEmpty.type] =
      FakeRequest.apply().withCookies(Cookie(CookieNames.deviceID, s"mdtpi#$randomString#blah"))

    val testDeviceIdCount: Int = 1001
    val testBuckets: List[Int] =
      for (
        i <- (1 to testDeviceIdCount).toList; buckets = BucketCalculator.deviceIdBucketCalculator(generateTestDeviceId)
      ) yield buckets
    ...
    val ChiSquared1000DegreesFreedom99PercentCertaintyBoundary: Double =
      888.564 // https://en.wikibooks.org/wiki/Engineering_Tables/Chi-Squared_Distibution
